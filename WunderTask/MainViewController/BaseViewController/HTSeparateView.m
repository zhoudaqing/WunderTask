...
- (UIView *)bottomLineView
{
    if (!_bottomLineView) {
        ...
        _bottomLineView.backgroundColor = [UIColor jd_lineColor];
    }
...
- (void)selectButtonAtIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    [_selectButton setTitleColor:[UIColor jd_globleTextColor] forState:UIControlStateNormal];
...
- (void)layoutSubviewsNow
{
    ...
    for (UIButton *button in _buttons) {
        ...
        if (index != (_buttons.count - 1)) {
            ...
            label.backgroundColor = [UIColor jd_lineColor];
        }
