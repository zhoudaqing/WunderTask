...
@implementation InvestingTableViewCell
...
- (void)addView
{
    ...
    rimeLine.frame = CGRectMake(15, view.height - 33, APPScreenWidth-30, 0.5);
...
- (UILabel *)abstractLable
{
    ...
    if (self.isRedeem) {
        UIImageView *image = [[UIImageView alloc]initWithFrame:CGRectMake(104, 0, 13, 13)];
        [image setImage:[UIImage imageNamed:@"transer"]];
        [_abstractLable addSubview:image];
    }
...
{
    if (!_moneyLable) {
        _moneyLable = [[UILabel alloc]initWithFrame:CGRectMake(15, 56.5, APPScreenWidth, 18)];
...
- (UILabel *)expectMoneyLable
{
    if (!_expectMoneyLable) {
        _expectMoneyLable = [[UILabel alloc]initWithFrame:CGRectMake(0, self.yieldLable.bottom + 13, APPScreenWidth - 15, 15)];
