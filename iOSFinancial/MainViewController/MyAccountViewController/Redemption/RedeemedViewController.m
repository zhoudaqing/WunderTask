...
@interface RedeemedViewController ()<MJRefreshBaseViewDelegate>
{
...
- (void)setNetWork
{
    HTBaseRequest *request = [HTBaseRequest requestWithURL:[NSString redeemedRcord]];
...
{
    _current_page++;
    NSString *url = [NSString stringWithFormat:@"%@?&page=%d",[NSString redeemedRcord],_current_page];
    return url;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.expectMoneyLable.text = [NSString  stringWithFormat:@"已经赎回:%@元", [dict stringFloatForKey:@"redeem_fee"]];
    
    cell.returnTimeLable.text = [NSString stringWithFormat:@"于%@申请赎回",[dict stringForKey:@"publish_time"]];
